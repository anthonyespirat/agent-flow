...
- **Model-dynamic dispatch.** `subagent-execution` picks a model per step — `sonnet` default, `haiku` for mechanical edits, `opus` only when the step carries an explicit upgrade signal. Unset defaults silently become expensive; the controller is deliberate.
